...
public class Soldier extends GeneralRobot {
	...
	public enum Mission {
		...
		NONE, PATROL, GOTO_ARCHON, GOTO_ENEMY, GOTO_LOCATION
	}
	...
	public void processMessage(ArrayList<Comms.CompoundMessage> cmsgs){
		for (CompoundMessage cmsg : cmsgs) {
			...
			case ATTACK:
				...
				loc = cmsg.loc;
				if (DEBUG) System.out.println("ATTACK: changed mission to GOTO_LOC");
				break;
			case GOTO:
	...
	public void action() throws GameActionException {
		...
		switch (mission) {
			case PATROL:
				if (myRC.getEnergonLevel() < myRC.getMaxEnergonLevel() * 0.4) {
					senseAlliedArchons();
			...
			case GOTO_ARCHON:
				...
				if (myRC.getEnergonLevel() > myRC.getMaxEnergonLevel() * 0.3) {
					mission = Mission.PATROL;
					break;
				}
				/* try not to get directly under Archon. It's high ground and workers like that spot */
	...
	private boolean goToLocation(MapLocation location) throws GameActionException {
		...
		if(myRC.getRoundsUntilMovementIdle() > 0) return false;
	...
	private boolean goToRobot(Robot robot) throws GameActionException {
		...
		if (info.location.isAdjacentTo(myRC.getLocation())){
			myRC.setDirection(myRC.getLocation().directionTo(info.location));
			return true;
		}
	...
	private void senseSurroundings() throws GameActionException {
